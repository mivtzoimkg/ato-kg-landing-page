        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="space-y-4">
              ...
              >
                <LazyVideo src="/videos/hanukkah_vid_1.MP4" autoPlay muted loop playsInline className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
            ...
            <div className="space-y-4 pt-8">
              ...
              <motion.div 
                ...
                className="rounded-[24px] overflow-hidden shadow-2xl border-2 border-white/30 aspect-video bg-black/40 relative group"
              >
                <LazyVideo src="/videos/hanukkah_vid_2.MP4" autoPlay muted loop playsInline className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                <div className="absolute inset-0 bg-linear-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
              </motion.div>
            </div>
          </motion.div>
...
const Footer = ({ setView }: { setView?: (v: 'home' | 'prayers') => void }) => {
  ...
  const [testResult, setTestResult] = useState<{ status: string, message?: string, details?: any, googleResponse?: any, info?: string } | null>(null);
  ...
  try {
    ...
    var date = data.date || new Date().toLocaleString("he-IL", { timeZone: "Asia/Jerusalem" });
    var fullName = data.fullName || "פלוני אלמוני";
    var email = data.email || "N/A";
    var amount = data.amount || 0;
    var source = data.source || "תרומה כללית";
    
    sheet.appendRow([date, fullName, email, amount, source]);
    
    return ContentService.createTextOutput(JSON.stringify({ 
      status: "success", 
      message: "המידע נשמר בהצלחה בגוגל שיטס!",
      row: [date, fullName, email, amount, source]
...
function doGet(e) {
  return ContentService.createTextOutput("הסקריפט פעיל בהצלחה!");
}`;
  ...
  const handleTestConnection = async (useCustomUrl: boolean) => {
    setTesting(true);
    setTestResult(null);
    try {
      const endpoint = useCustomUrl ? '/api/test-sheets-url' : '/api/test-sheets';
      const body = useCustomUrl ? JSON.stringify({ url: adminUrl }) : undefined;
      
      const response = await fetch(endpoint, {
        method: useCustomUrl ? 'POST' : 'GET',
        headers: useCustomUrl ? { 'Content-Type': 'application/json' } : undefined,
        body
      });
      ...
      setTestResult({ 
        status: 'error', 
        message: 'חיבור תקשורת לקוי או שגיאת שרת', 
        details: err.message 
      });
    } finally {
      setTesting(false);
  ...
  return (
    <footer className="bg-secondary text-white pt-20 pb-10 border-t border-white/5" dir="rtl">
      <div className="max-w-7xl mx-auto px-4">
        ...
        {/* Toggleable Admin Panel Connection Guides */}
        <div className="border-t border-white/10 pt-8 pb-4">
          <div className="flex justify-center mb-6">
            <button 
              onClick={() => setShowAdminPanel(!showAdminPanel)}
              className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/5 border border-white/10 hover:border-primary/50 text-gray-300 hover:text-white transition-all text-xs font-bold cursor-pointer"
            >
              <Settings size={14} className={showAdminPanel ? "animate-spin" : ""} />
              <span>{showAdminPanel ? "סגור לוח מנהל מערכת" : "חיבור ופתרון תקלות Google Sheets (למנהלי האתר)"}</span>
            </button>
          </div>
          ...
          <AnimatePresence>
            {showAdminPanel && (
              <motion.div 
                ...
                className="overflow-hidden bg-black/30 rounded-3xl border border-white/10 p-6 sm:p-8 text-right space-y-6 max-w-5xl mx-auto"
              >
                <div className="border-b border-white/10 pb-4">
                  <h4 className="text-xl font-bold text-primary mb-2 flex items-center gap-2">
                    <Code size={18} />
                    מדריך חיבור ואיתור תקלות - Google Sheets
                  </h4>
                  <p className="text-sm text-gray-300">
                    האתר תומך בשליחת נתוני תורמים ישירות לגיליון גוגל באמצעות Apps Script. להלן קוד הסקריפט המדויק והשלבים לפתרון תקלות.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-sm">
                  {/* Step 1 Instructions */}
                  <div className="space-y-4">
                    <h5 className="font-bold text-white text-base">שלבי הגדרה מנצחים (פעם אחת בלבד):</h5>
                    <ol className="list-decimal list-inside space-y-3 pr-2 text-gray-300">
                      <li>
                        פתחו את גיליון הגוגל שלכם.
                      </li>
                      <li>
                        בתפריט העליון לחצו על <strong>הרחבות (Extensions)</strong> &gt; <strong>Apps Script</strong>.
                      </li>
                      <li>
                        מחקו את כל הקוד הקיים שם ו<strong>הדביקו</strong> את קוד הסקריפט המוצג משמאל.
                      </li>
                      <li>
                        לחצו על אייקון ה<strong>שמירה</strong> (דיסקט) למעלה.
                      </li>
                      <li>
                        לחצו על כפתור <strong>Deploy (פריסה)</strong> בפינה הימנית העליונה &gt; <strong>New deployment (פריסה חדשה)</strong>.
                      </li>
                      <li>
                        לחצו על גלגל השיניים וביחרו ב-<strong>Web App (אפליקציית אינטרנט)</strong>.
                      </li>
                      <li>
                        שנו את השדה <strong>Who has access (למי יש גישה)</strong> ל-<strong>Anyone (כולם, גם ללא חשבון גוגל)</strong>. <span className="text-secondary font-bold font-mono text-xs">(קריטי למניעת שגיאת 403!)</span>
                      </li>
                      <li>
                        לחצו <strong>Deploy</strong>, אשרו הרשאות לחשבון הגוגל שלכם אם תתבקשו, ו<strong>העתיקו את ה-Web App URL</strong> שמופיע בסוף.
                      </li>
                    </ol>
                  </div>
                  ...
                  <div className="flex flex-col space-y-3">
                    <div className="flex justify-between items-center bg-zinc-800 rounded-t-xl px-4 py-2 border-b border-zinc-700">
                      <span className="text-xs text-gray-400 font-mono">Google Apps Script</span>
                      <button 
                        onClick={handleCopyCode}
                        className="flex items-center gap-1.5 text-xs text-primary bg-primary/10 hover:bg-primary hover:text-white px-2.5 py-1 rounded transition-colors cursor-pointer"
                      >
                        {copiedCode ? <Check size={12} /> : <Copy size={12} />}
                        <span>{copiedCode ? "הועתק בסלחת!" : "העתק קוד"}</span>
                      </button>
                    </div>
                    <pre className="bg-zinc-900 border border-zinc-700 rounded-b-xl p-4 overflow-x-auto text-[11px] font-mono text-zinc-300 text-left h-56 group select-all scrollbar-thin">
                      {appsScriptCode}
                    </pre>
                  </div>
                </div>
                ...
                <div className="bg-white/5 rounded-2xl border border-white/5 p-5 space-y-4">
                  <h5 className="font-bold text-white flex items-center gap-2">
                    ...
                    מרכז אבחון ובדיקת חיבור מעשיים:
                  </h5>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <button
                      ...
                      disabled={testing}
                      className="bg-primary hover:bg-primary-hover disabled:bg-primary/50 text-white font-bold px-6 py-2.5 rounded-xl transition-all cursor-pointer text-xs"
                    >
                      {testing ? "מבצע בדיקה..." : "בדוק חיבור נוכחי (המוגדר בשרת)"}
                    </button>

                    <div className="flex-1 flex gap-2">
                      <input
                        type="text"
                        placeholder="או הדביקו כאן כתובת סקריפט לבדיקה ידנית..."
                        value={adminUrl}
                        onChange={(e) => setAdminUrl(e.target.value)}
                        className="bg-black/40 border border-white/10 rounded-xl px-4 py-2 text-xs font-mono text-left w-full focus:outline-none focus:border-primary/50 text-white"
                      />
                      <button
                        onClick={() => handleTestConnection(true)}
                        disabled={testing || !adminUrl.startsWith("https://")}
                        className="bg-zinc-700 hover:bg-zinc-600 disabled:opacity-40 text-white px-4 py-2 rounded-xl transition-all cursor-pointer text-xs shrink-0"
                      >
                        בדוק ידנית
                      </button>
                    </div>
                  ...
                  {testResult && (
                    <div className={`rounded-xl border p-4 text-xs font-sans ${
                      testResult.status === 'success' 
                        ? 'bg-emerald-950/40 border-emerald-500/30 text-emerald-300' 
                        : 'bg-rose-950/40 border-rose-500/30 text-rose-300 font-sans'
                    }`}>
                      <div className="font-bold text-sm mb-1">
                        {testResult.status === 'success' ? '✓ החיבור הצליח ועובד נפלא!' : '✗ תקלה באיתור החיבור:'}
                      </div>
                      <p className="mb-2 leading-relaxed">{testResult.message || (testResult.status === 'success' ? 'שרת האפליקציה שלי מקבל ומנתב את המידע ללא שגיאות!' : '')}</p>
                      {testResult.details && (
                        <div className="bg-black/30 p-2.5 rounded-lg text-[11px] font-mono mb-2 text-left text-zinc-300 overflow-x-auto">
                          <strong>Response Details / Code:</strong> {typeof testResult.details === 'object' ? JSON.stringify(testResult.details) : testResult.details}
                        </div>
                      )}
                      {testResult.googleResponse && (
                        <div className="bg-black/30 p-2.5 rounded-lg text-[11px] font-mono text-left text-zinc-300 overflow-x-auto">
                          <strong>Google Script Output:</strong> {JSON.stringify(testResult.googleResponse)}
                        </div>
                      )}
                      {testResult.info && (
                        <p className="mt-2 text-gray-300 font-bold">{testResult.info}</p>
                      )}
                      
                      {testResult.status !== 'success' && (
                        <div className="mt-2 text-zinc-200">
                          <strong>כיצד לפתור?</strong>
                          <ul className="list-disc list-inside mt-1 space-y-1 pr-1 text-gray-300 text-xs">
                            <li>ודאו שהכתובת מתחילה ב-<code>https://script.google.com/macros/s/</code></li>
                            <li>ודאו שביצעתם <strong>Deploy -&gt; New deployment</strong> לאחר הדבקת הקוד.</li>
                            <li>חשוב מאוד: הגדירו את תפריט הגישה ל-<strong>Anyone</strong> (שורה 7 במדריך) - אחרת גוגל מסרבת לקבל נתונים מחשבונות חיצוניים.</li>
                          </ul>
                        </div>
                      )}
                    </div>
